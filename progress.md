...
## 会话：2026-08-14（v1.0.2-beta.2 双平台测试版）

### 表格密度、中文解释与 Linux 发行
- **状态：** in_progress
- 通过 Windows 应用控制读取用户当前运行的正式包，窗口约 1177x1041；确认字幕文件与输出目标
  路径均在实际数据下显示省略号，边界等窄字段默认占宽过多，序号列标题仍为“序号”。
- 映射表改为 `No.`；所有列继续允许手动调整。字幕文件和完整目标路径关闭省略并按完整内容定宽，
  窄字段按标题与当前值的最小完整宽度设置，窗口不足时使用水平滚动查看。
- 零点前事件中文说明改为解释：套用分集偏移后事件完全位于成片时间轴 0 秒之前，因此不会写入
  输出字幕；保留原始稳定诊断码和英文技术详情。
- 新增 Linux x86_64 PyInstaller onedir tar.gz，与 Windows x64 并行构建；Linux 使用 Xvfb+xcb
  验证真实 GUI 平台插件、无系统 Python 启动、中英文截图、SHA256 和包根许可证结构。
- 用户要求本测试版通过即发布；本轮只运行一次针对性本地回归与一次正式远程 CI/打包发布链，
  不重复 beta.1 已完成的本机全量构建流程。
- 本机唯一一轮验证完成：主窗口与版本 66 项测试全部通过，Ruff、64 个源码文件的严格 Mypy、
  workflow YAML 结构和 `git diff --check` 全部通过。2160x1500 中文 fixture 截图目视确认 `No.`、
  边界、区间、微调、置信度和状态列紧凑完整，字幕名与目标路径未出现省略号。
