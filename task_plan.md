...
## 当前阶段
阶段 5 收尾 / 阶段 6 发布准备：实际任务进度详情（远程测试替身契约修复中）

## 工作模型（2026-08-13 更新）
1. 远程绿色基线固定为提交 `38c3d7884a7941cc080bbfe22bbc47ccc5d0c759`、Actions run `31672669445`；当前未提交源码一律视为在研，不能沿用基线绿灯。
...
- [x] 补齐多目标输出交互
- [ ] 接入播放列表结构、源字幕详情和事件/样式/警告输出摘要
- [x] 修复精确 90 kHz offset、编辑/异步预检一致性与任务冻结缺口
- [x] 让取消信号贯穿长任务和事务写入
- [ ] 显示实际进度详情（扫描、字幕、预检、合并和写入已实现，待远程验证）
